...
void main() {
  ...
  test('bet, villain folds → we win his committed chips (not the whole pot)', () {
    final r = calc(_callsKingPlus).evaluate(unraised(), 14, [12], ActionType.bet);
    expect(r[12]!.ev, 1); // queen folds: his single chip comes to us
    expect(r[12]!.label, 'FOLDS');
  });

  test('bet, villain calls → ± the matched amount at showdown', () {
    final r = calc(_callsKingPlus).evaluate(unraised(), 14, [13], ActionType.bet);
    expect(r[13]!.ev, 3); // king calls the pot-to-3; ace wins matched 3
    expect(r[13]!.label, 'WIN');

    final behind = calc(_callsKingPlus).evaluate(unraised(), 9, [13], ActionType.bet);
    expect(behind[13]!.ev, -3); // a nine gets called and loses the matched 3
    expect(behind[13]!.label, 'LOSE');
  });
  ...
  test('call → ± the matched amount, per card', () {
    ...
    final r = calc(_callsKingPlus).evaluate(facingBet, 12, [14, 11], ActionType.call);
    expect(r[14]!.ev, -3); // queen loses to ace
    expect(r[14]!.label, 'LOSE');
    expect(r[11]!.ev, 3); // queen beats jack
    expect(r[11]!.label, 'WIN');
  });
  ...
  test('fold → we lose exactly what we already put in', () {
    ...
    final r = calc(_callsKingPlus).evaluate(facingBet, 12, [14], ActionType.fold);
    expect(r[14]!.ev, -1);
    expect(r[14]!.label, 'FOLD');
  });
  ...
  test('check: showdown when villain checks back, "?" when villain bets', () {
    ...
    final r = calc(potsAce).evaluate(unraised(), 13, [12, 14], ActionType.check);
    expect(r[12]!.ev, 1); // villain checks the queen → king wins the matched 1
    expect(r[12]!.label, 'WIN');
